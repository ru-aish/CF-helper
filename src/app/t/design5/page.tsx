...
export default function GamifiedDesign() {
  const [messages, setMessages] = useState([
    { role: 'assistant', content: "Great job! You found the pattern. Now, how do we sum this up efficiently for a very large N?" },
    { role: 'user', content: "Maybe we can group the numbers that have the same value for f(i)?" }
  ]);
  ...
  const handleSend = () => {
    ...
    setInput('');
    setTimeout(() => {
      setMessages(prev => [...prev, { role: 'assistant', content: "Excellent idea! 🌟 Let's see how many numbers in the range [1, N] share the same value for f(i)." }]);
    }, 1000);
  };
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto p-4 flex flex-col relative z-10">
        <div className="flex-1 bg-white/80 backdrop-blur-md rounded-[40px] border-[4px] border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-8 flex flex-col gap-6 overflow-y-auto mb-6">
          ...
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex gap-4 items-end ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              <div className={`w-12 h-12 rounded-[20px] flex items-center justify-center border-[3px] border-white shadow-sm pb-1 flex-shrink-0 ${msg.role === 'user' ? 'bg-[#FF9F43]' : 'bg-[#00D2D3]'}`}>
                {msg.role === 'user' ? (
              ...
              <div className={`px-6 py-4 rounded-[24px] font-medium text-[15px] shadow-sm max-w-[80%] ${msg.role === 'user' ? 'bg-[#0984E3] text-white rounded-br-sm' : 'bg-[#F1F2F6] text-[#2D3748] rounded-bl-sm border-[2px] border-[#E2E8F0]'}`}>
                {msg.content}
              </div>
            </div>
          ))}
        ...
        <div className="bg-white rounded-[32px] p-2 pr-3 flex items-center shadow-[0_10px_40px_rgba(0,0,0,0.08)] border-[3px] border-white">
           <button aria-label="Activity" className="w-12 h-12 flex items-center justify-center text-[#A0AEC0] hover:text-[#0984E3] hover:bg-[#F1F2F6] rounded-full transition-colors ml-1 flex-shrink-0">
           ...
           />
           <button onClick={handleSend} aria-label="Send message" className="bg-[#00D2D3] hover:bg-[#00b8b9] text-white p-4 rounded-[24px] shadow-sm transition-transform active:scale-95 flex items-center gap-2 font-bold px-6 flex-shrink-0">
             <span>Send</span>
